...
use polygraph::{
    result_mapping::{
        ColumnKind, CypherValue, ProjectedColumn, RdfTerm, SparqlSolution,
    },
...
fn schema_scalar_property_access() {
    let output = Transpiler::cypher_to_sparql_mapped(
...
fn schema_aliased_property() {
    let output = Transpiler::cypher_to_sparql_mapped(
        "MATCH (n:Person) RETURN n.name AS fullName",
        &ENGINE,
    )
    .unwrap();
...
fn schema_multiple_columns() {
    ...
    assert!(matches!(output.schema.columns[0].kind, ColumnKind::Node { .. }));
    assert_eq!(output.schema.columns[1].name, "b");
    assert!(matches!(output.schema.columns[1].kind, ColumnKind::Node { .. }));
    assert_eq!(output.schema.columns[2].name, "name");
    assert!(matches!(output.schema.columns[2].kind, ColumnKind::Scalar { .. }));
}
...
fn schema_aggregate_is_scalar() {
    ...
    assert!(matches!(output.schema.columns[0].kind, ColumnKind::Scalar { .. }));
}
...
#[test]
fn round_trip_sparql_string_preserved() {
    // The SPARQL string from the new API should match the old API.
    let old_sparql = Transpiler::cypher_to_sparql(
    ...
    .unwrap();
    assert_eq!(old_sparql, output.sparql);
}
